...
use crate::term::{TermId, TermIdTuple};

pub(crate) struct JoinTable {
    table: HashMap<Vec<TermId>, Vec<TermId>>,
    probe_indices: Vec<usize>,
}

impl JoinTable {
    pub(crate) fn new(probe_indices: impl IntoIterator<Item = usize>) -> Self {
        Self {
            table: HashMap::new(),
            probe_indices: probe_indices.into_iter().collect(),
        }
    ...
    pub(crate) fn insert(&mut self, term_id_tuple: TermIdTuple) {
        let mut probe_indices = self.probe_indices.iter().copied().peekable();
        let mut lookup = Vec::new();
        let mut expand = Vec::new();

        for (index, term_id) in term_id_tuple.iter().enumerate() {
            if Some(&index) == probe_indices.peek() {
                lookup.push(term_id);
                probe_indices.next();
            } else {
                expand.push(term_id);
    ...
    pub(crate) fn lookup(&mut self, term_ids: &[TermId]) -> &[TermId] {
        &self.table[&self
            .probe_indices
            .iter()
            .copied()
            .map(|index| term_ids[index])
            .collect::<Vec<_>>()]
    }
}
