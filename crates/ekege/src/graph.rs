...
impl QueryGraph {
    ...
    pub(crate) fn nodes(&self) -> impl Iterator<Item = (NodeId, Rc<FlatMapTermPattern>)> + use<'_> {
...
impl NodeSubset {
    ...
    pub(crate) fn non_empty_subsets(&self) -> impl Iterator<Item = NodeSubset> + use<'_> {
    ...
    pub(crate) fn nodes(&self) -> impl Iterator<Item = NodeId> + use<'_> {
    ...
    pub(crate) fn nodes_reverse(&self) -> impl Iterator<Item = NodeId> + use<'_> {
        let mut bitvector = self.node_bitvector;
