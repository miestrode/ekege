use cranelift::{
    ...
    prelude::{
        AbiParam, Configurable, FunctionBuilder, FunctionBuilderContext, InstBuilder, settings,
        types,
    },
};
...
impl QueryPlan {
    ...
    pub(crate) fn node(&self, index: PlanNodeIndex) -> Option<&PlanNode> {
        self.nodes.get(index)
    }

    pub(crate) fn node_mut(&mut self, index: PlanNodeIndex) -> Option<&mut PlanNode> {
